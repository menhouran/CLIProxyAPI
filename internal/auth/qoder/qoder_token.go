...
type QoderTokenStorage struct {
	...
	// recent /algo/api/v2/model/list response, keyed by display_name (e.g.
	// "Qwen3.7-Plus" -> {"key":"qmodel","display_name":"Qwen3.7-Plus",...}).
...
// The key can be either a display_name (direct lookup) or a server key
// (fallback: scans raw JSON for matching "key" field).
...
func (ts *QoderTokenStorage) GetModelConfig(key string) (json.RawMessage, bool) {
	...
	// Direct lookup (works for display_name keys)
	if raw, ok := ts.ModelConfigs[key]; ok {
		return raw, true
	}
	// Fallback: scan for server key in raw JSON
	return FindConfigByServerKey(ts.ModelConfigs, key)
}

// FindConfigByServerKey scans a display_name-keyed model config map for an
// entry whose raw JSON "key" field matches the given server key.
func FindConfigByServerKey(configs map[string]json.RawMessage, serverKey string) (json.RawMessage, bool) {
	needle := []byte(`"key":"` + serverKey + `"`)
	for _, raw := range configs {
		if len(raw) >= len(needle) {
			// Simple byte scan — avoids pulling in gjson just for this.
...
// ModelConfigKeys returns the list of cached model display names (used in
