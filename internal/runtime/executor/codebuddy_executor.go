...
// applyHeaders sets required headers for CodeBuddy chat API requests.
func (e *CodeBuddyExecutor) applyHeaders(req *http.Request, accessToken, userID, domain string) {
	...
	req.Header.Set("X-IDE-Version", "2.108.1")
...
// FetchCodeBuddyModels fetches the latest model list from the CodeBuddy config
// API and falls back to the static registry on error. The dynamic list is
// filtered to exclude internal / non-chat models.
